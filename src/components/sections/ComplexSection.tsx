  ...
  return (
    <section className="pt-[92px] bg-paper">
      ...
      <div className="relative bg-ink text-paper overflow-hidden">
        <Image src="/images/skyline-sunset.jpg" alt="" fill priority sizes="100vw" className="object-cover" />
        ...
        <div className="max-w-[1280px] mx-auto px-6 lg:pl-[88px] lg:pr-10 py-16 lg:py-24">
          ...
          {activeSubTab === "layout" && (
            <div>
              ...
              {/* SVG 단지 배치 다이어그램 */}
              <div className="mt-12 border border-ink/10 bg-paper p-8 lg:p-10">
                <div className="flex items-baseline justify-between mb-6">
                  <p className="text-rust text-[10.5px] tracking-[3px] uppercase">Block Diagram</p>
                  <p className="text-stone-light text-[10.5px] tracking-wider">11 BUILDINGS · 1,460 UNITS</p>
                </div>
                <svg viewBox="0 0 800 400" className="w-full h-auto" preserveAspectRatio="xMidYMid meet">
                  {/* 배경 그리드 */}
                  <defs>
                    <pattern id="grid" width="40" height="40" patternUnits="userSpaceOnUse">
                      <path d="M40 0H0V40" fill="none" stroke="currentColor" strokeWidth="0.5" className="text-ink/[0.04]" />
                    </pattern>
                  </defs>
                  <rect width="800" height="400" fill="url(#grid)" />

                  {/* 호수공원 (왼쪽) */}
                  <path d="M30 60 Q 100 40, 180 80 T 250 240 Q 200 320, 100 320 Q 30 280, 30 200 Z"
                    fill="currentColor" className="text-rust/15" stroke="currentColor" strokeWidth="1" />
                  <text x="120" y="200" className="fill-rust text-[11px]" textAnchor="middle" style={{ letterSpacing: "2px" }}>
                    성성호수공원
                  </text>

                  {/* 동 11개 — 직사각형으로 표현 */}
                  {[
                    { x: 320, y: 80, w: 30, h: 90, label: "101" },
                    { x: 360, y: 80, w: 30, h: 110, label: "102" },
                    { x: 400, y: 80, w: 30, h: 100, label: "103" },
                    { x: 440, y: 80, w: 30, h: 130, label: "104" },
                    { x: 480, y: 80, w: 30, h: 110, label: "105" },
                    { x: 520, y: 80, w: 30, h: 100, label: "106" },
                    { x: 320, y: 240, w: 30, h: 100, label: "107" },
                    { x: 360, y: 240, w: 30, h: 110, label: "108" },
                    { x: 400, y: 240, w: 30, h: 130, label: "109" },
                    { x: 440, y: 240, w: 30, h: 100, label: "110" },
                    { x: 480, y: 240, w: 30, h: 90, label: "111" },
                  ].map((b) => (
                    <g key={b.label}>
                      <rect x={b.x} y={b.y} width={b.w} height={b.h} fill="currentColor" className="text-ink/85" />
                      <text x={b.x + b.w / 2} y={b.y + b.h + 12} className="fill-stone text-[8px] tabular-nums" textAnchor="middle">
                        {b.label}
                      </text>
                    </g>
                  ))}
                  ...
                  <g className="text-rust">
                    <line x1="320" y1="220" x2="510" y2="220" stroke="currentColor" strokeWidth="1" strokeDasharray="3 3" />
                    <text x="415" y="232" className="fill-rust text-[9px] tabular-nums" textAnchor="middle">
                      MAX 125m
                    </text>
                  </g>

                  {/* 단지 입구 */}
                  <g className="text-stone-light">
                    <circle cx="600" cy="220" r="6" fill="none" stroke="currentColor" strokeWidth="1" />
                    <text x="600" y="245" className="fill-stone text-[9px]" textAnchor="middle">단지 입구</text>
                  </g>

                  {/* 도로 */}
                  <line x1="280" y1="60" x2="680" y2="60" stroke="currentColor" strokeWidth="1" className="text-ink/15" />
                  <line x1="280" y1="370" x2="680" y2="370" stroke="currentColor" strokeWidth="1" className="text-ink/15" />
                  <text x="680" y="55" className="fill-stone-light text-[9px]" textAnchor="end">번영로</text>
                  <text x="680" y="385" className="fill-stone-light text-[9px]" textAnchor="end">삼성대로</text>

                  {/* 방위표 */}
                  <g transform="translate(720,60)">
                    <circle r="20" fill="none" stroke="currentColor" strokeWidth="0.8" className="text-ink/30" />
                    <line x1="0" y1="-15" x2="0" y2="15" stroke="currentColor" strokeWidth="0.8" className="text-ink/30" />
                    <line x1="-15" y1="0" x2="15" y2="0" stroke="currentColor" strokeWidth="0.8" className="text-ink/30" />
                    <text y="-22" className="fill-rust text-[10px] font-bold" textAnchor="middle">N</text>
                  </g>
                </svg>
                <p className="text-stone-light text-[11px] mt-5 font-light">
                  * 본 다이어그램은 단지 배치 개념도이며, 실제 위치·동수·동간거리는 인허가 진행 상황에 따라 변경될 수 있습니다.
                </p>
              </div>
